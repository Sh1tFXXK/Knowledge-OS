...
export function validateGovernance(governance, data) {
  ...
  for (const plan of governance?.mergePlans ?? []) { assertUniqueId(plan, 'mergePlans'); if (!CANDIDATE_STATUSES.has(plan.status)) errors.push(`merge plan ${plan.id} has invalid status`); const source = plan.entityKind === 'edge' ? edgeIds : new Set(Object.keys(nodePool)); if (!source.has(plan.canonicalId)) errors.push(`merge plan ${plan.id} has missing canonical id: ${plan.canonicalId}`); for (const redundantId of plan.redundantIds ?? []) if (!source.has(redundantId)) errors.push(`merge plan ${plan.id} references missing redundant id: ${redundantId}`); }
