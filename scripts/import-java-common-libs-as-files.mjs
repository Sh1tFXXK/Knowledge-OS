...
const JAVA_NODE_ID = 'k_1782746457581_30q8ao';
...
// 子文件节点（语义化ID）
const CHILD_NODES = [
  { id: 'k_java_lang_string', treeId: 'tree_java_lang_string', name: 'java.lang.String', label: 'java.lang.String' },
  { id: 'k_java_lang_stringbuilder', treeId: 'tree_java_lang_stringbuilder', name: 'java.lang.StringBuilder', label: 'java.lang.StringBuilder' },
  { id: 'k_java_lang_stringbuffer', treeId: 'tree_java_lang_stringbuffer', name: 'java.lang.StringBuffer', label: 'java.lang.StringBuffer' },
  { id: 'k_java_util_scanner', treeId: 'tree_java_util_scanner', name: 'java.util.Scanner', label: 'java.util.Scanner' },
  { id: 'k_java_nio_file_files', treeId: 'tree_java_nio_file_files', name: 'java.nio.file.Files', label: 'java.nio.file.Files' },
  { id: 'k_java_lang_enum', treeId: 'tree_java_lang_enum', name: 'enum 枚举', label: 'enum（枚举）' },
];
...
async function main() {
  ...
  const javaTreeNode = findTreeNode(treeData, JAVA_TREE_ID);
  if (!javaTreeNode) throw new Error('Java tree node not found');

  // 1. 创建/更新分组节点
  const existingGroup = nodePool[GROUP_NODE_ID];
  nodePool[GROUP_NODE_ID] = {
    ...existingGroup,
    id: GROUP_NODE_ID,
    label: 'Java 常用类库',
    role: 'group',
    ...
    card: {
      ...
      title: 'Java 常用类库',
      tabs: [{ id: 'def', label: '说明', content: 'Java 标准库中最常用的类，包括字符串、输入扫描、文件操作、枚举等。' }],
    },
  };

  // 2. 在 java 树下创建/更新分组树节点
  let groupTree = findTreeNode(treeData, GROUP_TREE_ID);
  if (!groupTree) {
    groupTree = {
      id: GROUP_TREE_ID,
      name: '常用类库',
      count: 0,
      nodeRef: GROUP_NODE_ID,
      children: [],
    };
    javaTreeNode.children ??= [];
    javaTreeNode.children.push(groupTree);
  }
  groupTree.nodeRef = GROUP_NODE_ID;
  groupTree.children ??= [];

  // 3. 创建子文件节点并挂载
  for (const child of CHILD_NODES) {
    const knowledge = KNOWLEDGE[child.id];
    if (!knowledge) {
      console.warn(`No knowledge for ${child.id}`);
      continue;
    }
    ...
    nodePool[child.id] = {
      ...
      role: 'concept',
      dimensions: ['java', 'jdk-api'],
      tags: ['java', 'jdk', '常用类库', child.name],
      card: {
        ...(existing?.card ?? {}),
        nodeId: child.id,
        title: child.label,
        tabs: makeTabs(knowledge),
      },
    };
    ...
    upsertEdge(knowledgeEdges, {
      id: `treebind:${GROUP_TREE_ID}:${child.treeId}`,
  ...
  const semanticEdges = [
    {
      ...
      source: 'k_java_lang_string',
      target: 'k_java_lang_stringbuilder',
      type: 'contrasts-with',
      label: '不可变 vs 可变',
      dimensions: ['java'],
    ...
    },
    {
      ...
      source: 'k_java_lang_string',
      target: 'k_java_lang_stringbuffer',
      type: 'contrasts-with',
      label: '不可变 vs 可变',
      dimensions: ['java'],
      relationKind: 'comparison',
    },
    {
      ...
      source: 'k_java_lang_stringbuilder',
      target: 'k_java_lang_stringbuffer',
      type: 'contrasts-with',
      label: '非线程安全 vs 线程安全',
      dimensions: ['java'],
    ...
    {
      id: 'edge_java_scanner_reads_string',
      source: 'k_java_util_scanner',
      target: 'k_java_lang_string',
      type: 'produces',
      label: '读取输入为字符串',
      dimensions: ['java'],
  ...
  // 更新分组 count
  groupTree.count = CHILD_NODES.length;
  ...
  console.log(`Created group + ${CHILD_NODES.length} child nodes, ${semanticEdges.length} semantic edges.`);
}
